...
class Queen(Piece):
    CANDIDATE_MOVE_VECTORS = [-9, -8, -7, -1, 1, 7, 8, 9]

    def __init__(self, color, position, is_first_move):
        super().__init__('queen', color, position, is_first_move)

    def calculate_legal_moves(self, board):
        legal_moves = []

        for vector in Queen.CANDIDATE_MOVE_VECTORS:
            curr_position = self.position

            # So long as we haven't haven't reached the edges of the board
            while not (Queen._is_first_column_exclusion(curr_position, vector) or
                       Queen._is_eighth_column_exclusion(curr_position, vector) or
                       Queen._is_first_row_exclusion(curr_position, vector) or
                       Queen._is_eighth_row_exclusion(curr_position, vector)):
