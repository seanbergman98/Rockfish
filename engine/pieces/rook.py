...
class Rook(Piece):
    CANDIDATE_MOVE_VECTORS = [-8, -1, 1, 8]
    ...
    def calculate_legal_moves(self, board):
        ...
        for vector in Rook.CANDIDATE_MOVE_VECTORS:
            curr_position = self.position

            # So long as we haven't haven't reached the edges of the board
            while not (Rook._is_first_column_exclusion(curr_position, vector) or
                       Rook._is_eighth_column_exclusion(curr_position, vector) or
                       Rook._is_first_row_exclusion(curr_position, vector) or
                       Rook._is_eighth_row_exclusion(curr_position, vector)):
